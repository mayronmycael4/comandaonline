...
async function removerItem(itemId) {
    ...
    const motivo = prompt('Informe o motivo da remocao deste item:');
    if (motivo === null) return;
    const motivoTrim = String(motivo || '').trim();
    if (!motivoTrim) {
        Toast.warning('Motivo obrigatorio para remover item.');
        return;
    }

    if (!currentComanda.motivosRemocao || typeof currentComanda.motivosRemocao !== 'object') {
        currentComanda.motivosRemocao = {};
    }
    currentComanda.motivosRemocao[String(itemId)] = motivoTrim;
...
async function salvarComanda(options = {}) {
    ...
    try {
        ...
        if (result && result.pendingSync) {
            Toast.warning('Comanda salva em pendencia. Tentando enviar automaticamente...');
...
async function reabrirComanda() {
    ...
    const motivoTrim = String(motivo || '').trim();
    if (!motivoTrim) {
        Toast.warning('Motivo obrigatorio para reabrir comanda.');
        return;
    }
    
    try {
        const result = await Storage.reabrirComanda(currentComanda.id, motivoTrim);
...
async function imprimirComanda() {
    ...
    const empresaNome = empresa?.nome ? String(empresa.nome).toUpperCase() : 'COMANDA ONLINE';
