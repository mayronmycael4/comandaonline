...
function ensureKitchenColumns(PDO $pdo): array {
    ...
    $toAdd = [
        ...
        'enviado_cozinha_em' => 'TIMESTAMP NULL DEFAULT NULL',
...
if ($method === 'GET') {
    ...
    $sql = "
        SELECT SQL_NO_CACHE
            ...
            TIMESTAMPDIFF(MINUTE, ci.created_at, UTC_TIMESTAMP()) AS item_minutos_decorridos,
            c.numero_mesa,
            ...
            TIMESTAMPDIFF(MINUTE, c.created_at, UTC_TIMESTAMP()) AS comanda_minutos_decorridos,
...
elseif ($method === 'PUT' || $method === 'POST') {
    ...
    if (!empty($data['item_id'])) {
        ...
        if ($hasKitchenProntoAt) {
            if ($statusNovo === 'pronto') {
                $prontoAt = ', kitchen_pronto_at = UTC_TIMESTAMP()';
    ...
    if (!empty($data['comanda_id'])) {
        $prontoAt = $hasKitchenProntoAt ? ', kitchen_pronto_at = UTC_TIMESTAMP()' : '';
        $stmt = $pdo->prepare("
            UPDATE comanda_itens
