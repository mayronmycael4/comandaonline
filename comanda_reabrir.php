...
$motivoReabertura = trim((string)($data['motivo'] ?? ''));

if (!$comandaId) {
    jsonResponse(['error' => 'ID da comanda é obrigatório'], 400);
}

if (!actorHasPermission($pdo, $actor, 'COMANDA_REABRIR')) {
    denyAndAudit($pdo, $actor, 'COMANDA_REABRIR', 'comandas', $comandaId, [
        'acao' => 'reabrir_comanda'
    ]);
}

if ($motivoReabertura === '') {
    jsonResponse(['error' => 'Motivo da reabertura é obrigatório'], 400);
}
...
try {
    ...
    registrarHistoricoStatusComanda($pdo, (int)$comandaId, $statusAnterior, 'aberta', $actor, $motivoReabertura);
    ...
    if ($comanda['cliente_id']) {
        ...
        if ($cliente) {
            $stmt = $pdo->prepare("
                ...
                WHERE cliente_id = ? AND comanda_id = ? AND pontos_ganhos > 0
            ");
            $stmt->execute([$comanda['cliente_id'], $comandaId]);
            $pontosRemovidos = (int)$stmt->fetchColumn();
            ...
            ");
            $stmt->execute([$comanda['cliente_id'], $comandaId]);
            $jaEstornada = (int)$stmt->fetchColumn() > 0;
