    ...
    if (formSetup) {
        ...
        try {
            ...
            try {
                ...
                await prepareFreshSessionEnvironment();
                Storage.setSession(loginResult.funcionario);
                if (typeof Storage.preloadCoreDataForSession === 'function') {
                    Storage.preloadCoreDataForSession();
                } else if (typeof Storage.preloadFuncionariosForSession === 'function') {
                    Storage.preloadFuncionariosForSession();
                }
            } catch (loginError) {
...
async function prepareFreshSessionEnvironment() {
    const preservedEntries = preserveImportantLocalSettings();
