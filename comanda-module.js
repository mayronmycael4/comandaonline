const ComandaModule = (() => {
    ...
    function buildActionMenu(comanda, options = {}) {
        ...
        const pendingSync = !!comanda.pendingSync || String(comanda.id || '').startsWith('local-');
        const items = [
        ...
        if (options.allowToggleStatus !== false && !pendingSync) {
            if (status === 'aberta') {
            ...
            items.push('<button class="comanda-action-item" type="button" disabled>Sincronizando...</button>');
        }
