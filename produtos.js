...
function bindProductEvents() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const setor = document.getElementById('setorProduto').value || 'cozinha';
        ...
        try {
            await Storage.addProduto({ nome, categoria, preco, setor, descricao });
            form.reset();
            await carregarProdutos();
            ...
            <div class="produto-item produto-card" style="border-left: 4px solid #ffc107;">
                <div class="comanda-card-header">
                    <div>
                        ...
                        <p style="margin-top: 4px; font-size: 0.82rem;">Categoria: ${categoriaLabel} | Setor: ${escapeHtml((produto.setor || 'cozinha').toUpperCase())}</p>
                        ${produto.descricao ? `<p style="margin-top:4px; font-size:0.8rem; opacity:0.85;">${escapeHtml(produto.descricao)}</p>` : ''}
                    </div>
...
function abrirModalEdicao(id) {
    ...
    document.getElementById('editarSetorProduto').value = produto.setor || 'cozinha';
...
async function salvarEdicaoProduto(event) {
    ...
    const setor = document.getElementById('editarSetorProduto').value || 'cozinha';
    const descricao = document.getElementById('editarDescricaoProduto').value.trim();
    ...
    try {
        await Storage.updateProduto({ id, nome, categoria, preco, setor, descricao });
        fecharModalEdicao();
        await carregarProdutos();
