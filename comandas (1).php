...
function calcularStatusOperacionalComanda(PDO $pdo, array $comanda, bool $hasKitchenStatus): string {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) AS total, " .
        ($hasKitchenStatus ? "SUM(CASE WHEN kitchen_status = 'pendente' THEN 1 ELSE 0 END) AS pendentes, SUM(CASE WHEN kitchen_status = 'pronto' THEN 1 ELSE 0 END) AS prontos" : "0 AS pendentes, 0 AS prontos") .
...
function detectItemCols(PDO $pdo): array {
    ...
    $toAdd = [
        'kitchen_status'    => "ENUM('pendente','pronto') NOT NULL DEFAULT 'pendente'",
...
switch ($method) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                SELECT id, produto_id, nome_item, categoria, quantidade, valor_unitario,
                       kitchen_status, kitchen_pronto_at, observacoes
                FROM comanda_itens
            ...
            foreach ($stmt->fetchAll() as $row) {
                ...
                if ($_hasKitchenStatus && ($row['kitchen_status'] ?? null) === 'pronto') {
            ...
            foreach ($itensRecebidos as $item) {
                ...
                $kStatus       = isset($prontos[$itemIdCliente]) ? 'pronto' : 'pendente';
                $kProntoAt     = $prontos[$itemIdCliente] ?? null;

                [$sql, $params] = buildItemInsert(
                    $pdo, $item, $id,
                    $_hasObservacoes, $_hasKitchenStatus, $_hasKitchenProAt,
                    $kStatus, $kProntoAt
                );
            ...
            foreach ($itensCancelados as $itemCancelado) {
                [$sql, $params] = buildItemInsert(
                    ...
                    $_hasKitchenStatus,
                    $_hasKitchenProAt,
                    $_hasKitchenStatus && ($itemCancelado['kitchen_status'] ?? null) === 'pronto' ? 'pronto' : 'pendente',
    ...
    case 'DELETE':
        ...
        try {
            ...
            registrarHistoricoStatusComanda($pdo, (int)$id, $statusAnterior, 'cancelada', $actor, 'cancelamento_comanda');
