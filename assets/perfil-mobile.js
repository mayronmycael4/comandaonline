...
function renderAtalhosGestao() {
    ...
    const links = [
        { permission: 'produtos', href: 'produtos-mobile.html', label: 'Produtos', desc: 'Cadastrar e editar produtos', icon: '🍔' },
        { permission: 'estoque', href: 'estoque-mobile.html', label: 'Estoque', desc: 'Controle e lista de compras', icon: '📦' },
        { permission: 'comandas', href: 'qr-mesas.html', label: 'QR de Mesas', desc: 'Gerar e imprimir QR Code por mesa', icon: '📱' },
        { permission: 'SISTEMA_VER_LOGS', href: 'monitoramento.html', label: 'Monitoramento', desc: 'Logs e auditoria do sistema', icon: '⚙️' },
        { permission: 'relatorios', href: 'relatorios-mobile.html', label: 'Relatórios', desc: 'Financeiro e desempenho', icon: '📈' },
        { permission: 'funcionarios', href: 'funcionarios-mobile.html', label: 'Funcionários', desc: 'Usuários e permissões', icon: '👥' }
    ...
    grid.innerHTML = links.map((item) => `
        <a class="admin-shortcut-card" href="${item.href}">
            <span class="admin-shortcut-icon" aria-hidden="true">${item.icon || '•'}</span>
