...
const CozinhaModule = (() => {
    ...
    function _isPendente(status) {
        return ['pendente', 'recebido', 'em_preparo'].includes(String(status || '').toLowerCase());
    }

    function _isProntoOuEntregue(status) {
        return ['pronto', 'entregue'].includes(String(status || '').toLowerCase());
    }
    ...
    let _setorFiltro  = '';    // '' | 'cozinha' | 'churrasqueira' | 'bar'
    let _callbacks    = {};
    ...
    async function _fetchPedidos() {
        const url = _viewMode === 'todos'
            ? `cozinha.php?todos=1${_setorFiltro ? `&setor=${encodeURIComponent(_setorFiltro)}` : ''}`
    ...
    async function marcarItemPronto(itemId, comandaId, status = 'pronto') {
        const resp = await fetch('cozinha.php', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ item_id: itemId, comanda_id: comandaId || undefined, status })
        });
    ...
    function _calcMinutosGrupo(grupo) {
        const itensAtivos = (grupo.itens || []).filter((item) => item.kitchen_status !== 'pronto');

        // Fonte primaria: minutos calculados no backend via SQL (mesma base de tempo do banco).
        const minutosItens = itensAtivos
            .map((item) => Number(item.item_minutos_decorridos))
            .filter((v) => Number.isFinite(v));

        if (minutosItens.length > 0) {
            return Math.max(0, Math.max(...minutosItens));
        }

        const minutosComanda = Number(grupo.comanda_minutos_decorridos);
        if (Number.isFinite(minutosComanda)) {
            return Math.max(0, minutosComanda);
        }

        // Fallback: calculo local para compatibilidade com payloads antigos.
        return _calcMinutos(_getGrupoReferenciaTempo(grupo));
    }
    ...
    function _detectarNovos(novosPedidos) {
        ...
        novosPedidos.forEach(grupo => {
            grupo.itens.forEach(item => {
                ...
                if ((_isPendente(item.kitchen_status) || item.kitchen_status === 'cancelado') && !anteriores.has(chave)) {
                    novosIds.push(item.item_id);
                    if (_isPendente(item.kitchen_status)) {
    ...
    function _renderItem(item, comandaId, numeroMesa) {
        ...
        const statusLabel = _isProntoOuEntregue(item.kitchen_status)
            ? (item.kitchen_status === 'entregue' ? '✔ Entregue' : '✔ Pronto')
        ...
        const obs = item.observacoes
            ...
            : '';
        const proximoStatus = String(item.kitchen_status || '').toLowerCase() === 'em_preparo' ? 'pronto' : 'em_preparo';
        const acaoLabel = proximoStatus === 'pronto' ? '✔' : 'Preparar';
        const acaoTitle = proximoStatus === 'pronto' ? 'Marcar item como pronto' : 'Marcar item em preparo';
        const acaoItem = _isPendente(item.kitchen_status)
            ? `<button type="button" class="btn-pronto-item"
                  ...
                  title="${acaoTitle}">${acaoLabel}</button>`
    ...
    function _getSetoresDoGrupo(grupo) {
        const setores = (grupo.itens || [])
            .map((item) => String(item.kitchen_setor || 'cozinha').trim() || 'cozinha')
            .filter((value, index, array) => array.indexOf(value) === index);
        return setores.length > 0 ? setores : ['cozinha'];
    }

    function _renderCard(grupo, isNovo) {
        const minutos  = _calcMinutosGrupo(grupo);
        const classe   = _classePrioridade(minutos);
        const timer    = _textoTimer(minutos);
        const temPendente = grupo.itens.some(i => _isPendente(i.kitchen_status));
        ...
        const setores = _getSetoresDoGrupo(grupo);
        const impressoes = setores.map((setor) => `
            <button type="button" class="btn-pronto-item btn-imprimir-setor" data-print-setor="${_esc(setor)}" data-comanda-id="${grupo.comanda_id}">
                Imprimir ${_esc(setor)}
            </button>
        `).join('');
    ...
    function _renderPorCategoria(todosGrupos) {
        ...
        Object.keys(mapa).sort().forEach(cat => {
            ...
            mapa[cat].forEach(item => {
                ...
                const st    = _isProntoOuEntregue(item.kitchen_status) ? 'item-pronto' : (item.kitchen_status === 'cancelado' ? 'item-cancelado' : 'item-pendente');
                const stLbl = _isProntoOuEntregue(item.kitchen_status) ? (item.kitchen_status === 'entregue' ? '✔ Entregue' : '✔ Pronto') : (item.kitchen_status === 'cancelado' ? 'Cancelado' : 'Pendente');
                const canceladoClass = item.kitchen_status === 'cancelado' ? ' pedido-cancelado' : '';
                const setorAtual = item.kitchen_setor || 'cozinha';
                html += `
                    <div class="cozinha-card prioridade-nova${canceladoClass}">
                        <div class="cozinha-card-header">
                            <div class="cozinha-mesa">${item.quantidade}× ${_esc(item.nome_item)}<small>Mesa ${_esc(String(item._mesa))} • ${_esc(setorAtual)}</small></div>
                        ...
                        ` : _isPendente(item.kitchen_status) ? `
                            <div class="cozinha-card-actions" style="margin-top:10px">
                                <button type="button" class="btn-pronto-item" data-item-id="${item.item_id}" data-comanda-id="${item._cid}" data-status="${String(item.kitchen_status || '').toLowerCase() === 'em_preparo' ? 'pronto' : 'em_preparo'}">${String(item.kitchen_status || '').toLowerCase() === 'em_preparo' ? '✔ Pronto' : 'Preparar'}</button>
                            </div>
                        ` : ''}
                    </div>`;
            });
    ...
    function render(container, pedidos, novosIds) {
        ...
        container.querySelectorAll('.btn-pronto-item').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                const status = btn.getAttribute('data-status') || 'pronto';
                btn.disabled = true;
                btn.textContent = '⏳';
                try {
                    await marcarItemPronto(iid, cid, status);
                    if (_callbacks.onUpdate) await _callbacks.onUpdate();
                    // Notificar garçom se todos os itens da comanda estão prontos
                    const grupoAtual = _listaPedidos.find(g => g.comanda_id === cid);
                    if (!grupoAtual || grupoAtual.itens.every(i => _isProntoOuEntregue(i.kitchen_status))) {
        ...
        container.querySelectorAll('.btn-imprimir-setor').forEach(btn => {
            btn.addEventListener('click', async () => {
                const cid = parseInt(btn.getAttribute('data-comanda-id'), 10);
                const setor = String(btn.getAttribute('data-print-setor') || '').trim();
                if (!cid || !setor) return;
                btn.disabled = true;
                const original = btn.textContent;
                btn.textContent = '⏳';
                try {
                    const res = await Storage.gerarKdsTicketSetor(cid, setor);
                    if (res && res.ticket && res.ticket.id) {
                        await Storage.confirmarImpressaoKds(res.ticket.id);
                    }
                    if (res && res.duplicado) {
                        Toast.info('Ticket já gerado para esse setor nesta mesma composição.');
                    } else {
                        Toast.success(`Ticket do setor ${setor} gerado.`);
                    }
                } catch (e) {
                    Toast.error('Falha ao gerar impressão do setor.');
                } finally {
                    btn.disabled = false;
    ...
    function atualizarContador(contadorEl, pedidos) {
        ...
        const totalPendentes = pedidos.reduce((sum, g) => sum + g.itens.filter(i => _isPendente(i.kitchen_status)).length, 0);
    ...
    function bindToolbar(container, onUpdate) {
        ...
        container.querySelectorAll('[data-kds-setor]').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-kds-setor') === _setorFiltro);
            btn.addEventListener('click', () => {
                _setorFiltro = btn.getAttribute('data-kds-setor') || '';
                container.querySelectorAll('[data-kds-setor]').forEach(b => b.classList.toggle('active', b === btn));
                if (onUpdate) onUpdate();
            });
        });
    }
