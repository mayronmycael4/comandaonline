...
if (!$env:COMANDA_QA_EMAIL -or !$env:COMANDA_QA_PASSWORD) { throw 'Configure COMANDA_QA_EMAIL e COMANDA_QA_PASSWORD antes do teste.' }
...
$loginResult = Invoke-WebRequest ($base+'login.php') -WebSession $qaSession -Method Post -Body @{_token=$token;email=$env:COMANDA_QA_EMAIL;senha=$env:COMANDA_QA_PASSWORD} -UseBasicParsing
