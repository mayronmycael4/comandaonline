...
if ($method === 'GET') {
    ...
    $selKitchenStatus   = $hasKitchenStatus   ? 'ci.kitchen_status,'             : "'pendente' AS kitchen_status,";
    ...
    foreach ($rowsPorComanda as $cid => $rowsComanda) {
        ...
        foreach ($rowsComanda as $row) {
            ...
            if ($somentePendentes && $kitchenStatus === 'pronto') {
                continue;
            }
...
elseif ($method === 'PUT' || $method === 'POST') {
    ...
    if (!empty($data['item_id'])) {
        ...
        auditLog($pdo, 'cozinha_item_pronto', 'comanda_itens', (int)$data['item_id'], [
            'comanda_id' => isset($data['comanda_id']) ? (int)$data['comanda_id'] : null
        ], $actor);
        jsonResponse(['success' => true, 'updated' => 'item', 'item_id' => (int) $data['item_id']]);
    }
    ...
    if (!empty($data['comanda_id'])) {
        ...
        $stmt = $pdo->prepare("
            UPDATE comanda_itens
            SET kitchen_status = 'pronto'{$prontoAt}
            WHERE comanda_id = ? AND kitchen_status = 'pendente'
        ");
