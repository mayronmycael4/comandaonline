...
function bindExportButton() {
    ...
    btn.addEventListener('click', async () => {
        try {
            const url = buildCsvExportUrlForCurrentTab();
            if (!url) {
                Toast.warning('Selecione um período válido para exportar.');
                return;
            }
...
function buildCsvExportUrlForCurrentTab() {
    const hoje = new Date();
    const toISODate = (d) => {
    ...
    if (currentTab === 'dia') {
        const data = toISODate(hoje);
        return API.getRelatorioCsvUrl('dia', { data });
    }

    if (currentTab === 'semana') {
        return API.getRelatorioCsvUrl('semana');
    }

    if (currentTab === 'mes') {
        const mes = `${hoje.getFullYear()}-${String(hoje.getMonth() + 1).padStart(2, '0')}`;
        return API.getRelatorioCsvUrl('mes', { mes });
    }

    if (currentTab === 'ano') {
        return API.getRelatorioCsvUrl('ano', { ano: hoje.getFullYear() });
    }

    if (currentTab === 'personalizado') {
        ...
        return API.getRelatorioCsvUrl('periodo', { inicio, fim });
    }
